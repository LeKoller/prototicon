  ...
  setup() {
    ...
    async function getTimeline() {
      ...
      for (let username of usernames) {
        await axios
          .get(
            `http://ec2-177-71-148-166.sa-east-1.compute.amazonaws.com/api/contents/?author=${username}`,
            config
          )
    ...
    function getMessagesOnCache() {
      try {
        for (let friend of store.state.user.friends) {
          axios
            .get(
              `http://ec2-177-71-148-166.sa-east-1.compute.amazonaws.com/api/messages/?username=${friend.username}`,
              config
            )
            .then((res) => {
    ...
    onMounted(() => {
      ws.value.current = new WebSocket(
        `ws://ec2-177-71-148-166.sa-east-1.compute.amazonaws.com/ws/messages/?token=${store.state.user.token}`
      );
